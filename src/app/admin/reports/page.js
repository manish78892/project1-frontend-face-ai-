...
export default function AttendanceRangeReport() {
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fb] flex flex-col font-sans text-slate-800">
      ...
      {/* Main Content */}
      <main className="flex-grow w-full max-w-[1400px] mx-auto px-4 md:px-6 pt-6 md:pt-8">
        <div className="max-w-7xl mx-auto">
          {/* Description */}
          <p className="text-slate-500 text-sm md:text-base mb-6">View attendance by date range.</p>

          {/* Date Controls – Stack on mobile */}
          <div className="bg-white rounded-xl p-4 md:p-6 border border-slate-100 shadow-sm mb-8 flex flex-col md:flex-row gap-4 items-stretch md:items-end">
            <div className="flex-1">
              ...
              <input
                ...
                className="w-full px-4 py-2 border border-slate-200 rounded-xl text-sm text-slate-900"
              />
            </div>
            <div className="flex-1">
            ...
            <button
              onClick={fetchReport}
              disabled={loading}
              className="bg-blue-600 text-white px-6 py-2 rounded-xl font-semibold hover:bg-blue-700 transition disabled:opacity-50 md:self-end"
            >
          ...
          {report && (
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-x-auto">
              <div className="p-3 md:p-4 border-b border-slate-100 font-semibold text-xs md:text-sm text-slate-700">
              ...
              <table className="w-full text-xs md:text-sm whitespace-nowrap">
                <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-100">
                  <tr>
                    <th className="px-3 py-2 text-left sticky left-0 bg-slate-50">Name</th>
                    <th className="px-3 py-2 text-left">Faculty Login ID</th>
                    <th className="px-3 py-2 text-left">Dept</th>
                    {report.days.map(day => (
                      <th key={day} className="px-2 py-2 text-center text-[10px] font-bold uppercase tracking-wider">
                ...
                <tbody>
                  {report.matrix.map(row => (
                    <tr key={row.facultyId} className="border-b border-slate-50 hover:bg-slate-50/50">
                      <td className="px-3 py-2 font-semibold text-slate-900 sticky left-0 bg-white">{row.name}</td>
                      <td className="px-3 py-2 font-mono text-xs text-slate-500">{row.facultyId}</td>
